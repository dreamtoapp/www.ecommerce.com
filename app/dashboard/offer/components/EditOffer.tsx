...
import { updateOffer } from '@/app/dashboard/offer/actions';
...
const OfferForm = ({ id, name, logo }: { name: string; logo: string | null; id: string }) => {
  ...
  const [state, addAction, isPending] = useActionState(updateOffer, {
    success: false,
    message: '',
  });
...
export default function EditOffer({ supplier }: EditSupplierDialogProps) {
  return (
    <Dialog>
      ...
      >
        <DialogHeader>
          <DialogTitle className='text-right text-lg font-semibold'>تعديل العرض</DialogTitle>
          <DialogDescription className='text-right'>قم بتعديل تفاصيل العرض هنا</DialogDescription>
        </DialogHeader>

        <OfferForm id={supplier.id} name={supplier.name} logo={supplier.logo} />
      </DialogContent>
    </Dialog>
  );
}
