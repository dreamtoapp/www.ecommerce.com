...
export default function PromotionSeoForm({ promotion, seoByLocale }: { promotion: any, seoByLocale: Record<string, any> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    const { updatePromotionSeo } = await import('../actions/update-promotion-seo');
    const res = await updatePromotionSeo({
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-3xl mx-auto flex flex-col gap-8" autoComplete="off">
      <Card className="bg-background border border-border shadow-xl rounded-2xl">
        <CardHeader>
          <CardTitle className="text-xl font-semibold text-foreground">SEO العرض ({LOCALES.find(l => l.code === activeLocale)?.label})</CardTitle>
          <CardDescription className="text-muted-foreground text-base">
            جميع الحقول مطلوبة لتحسين ظهور العرض في نتائج البحث ({activeLocale === 'ar-SA' ? 'العربية' : 'English'}).
          </CardDescription>
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                ...
                <InfoTooltip content="العنوان الرئيسي للعرض كما يظهر في نتائج البحث. يجب أن يكون فريدًا وجذابًا ويحتوي على الكلمات المفتاحية الأساسية." />
              </div>
              <Input
                type="text"
                required
                ...
                placeholder={activeLocale === 'ar-SA' ? 'مثال: عرض خاص' : 'e.g. Special Promotion Title'}
                autoComplete="off"
              />
            ...
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                ...
                <InfoTooltip content="وصف مختصر للعرض يظهر أسفل العنوان في نتائج البحث. يجب أن يكون مشوقًا ويوضح محتوى العرض." />
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  onClick={() => {
                    if (promotion?.title) {
                      setForm((prev: any) => ({
                        ...
                        [activeLocale]: {
                          ...prev[activeLocale],
                          metaDescription: `استفد من عرض ${promotion.title} الآن! خصومات حصرية لفترة محدودة.`,
                        },
                      }));
                    }
                  }}
              ...
              <Textarea
                ...
                placeholder={activeLocale === 'ar-SA' ? 'وصف مختصر وجذاب للعرض...' : 'Short and catchy promotion description...'}
            ...
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                ...
                <InfoTooltip content="العنوان الذي يظهر عند مشاركة العرض على وسائل التواصل الاجتماعي." />
              </div>
              <Input
                type="text"
            ...
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                ...
                <InfoTooltip content="وصف مختصر يظهر عند مشاركة العرض على وسائل التواصل الاجتماعي." />
              </div>
              <Textarea
            ...
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                ...
                <InfoTooltip content="الرابط الأساسي للعرض لمنع تكرار المحتوى في محركات البحث. اتركه فارغًا إذا لم تكن متأكدًا." />
              </div>
              <Input
                type="url"
                value={form[activeLocale].canonicalUrl}
                onChange={e => handleChange(activeLocale, 'canonicalUrl', e.target.value)}
                placeholder="https://www.ammawag.com/promotion/slug"
              />
            </div>
            {/* robots */}
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                ...
                <InfoTooltip content="تعليمات لمحركات البحث حول كيفية فهرسة العرض. اختر من الخيارات القياسية فقط." />
              </div>
              <select
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              <span className="text-xs text-muted-foreground">اختر النوع المناسب حسب محتوى العرض.</span>
            </div>
            {/* schemaOrg */}
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-row items-center gap-2 w-full">
                <label className="font-semibold text-sm text-foreground">Schema.org JSON-LD</label>
                <InfoTooltip content="كود JSON-LD لتعريف بيانات العرض لمحركات البحث (اختياري، متقدم). إذا لم تكن متأكدًا من كيفية إنشاء هذا الكود، يمكنك تركه فارغًا أو توليد تلقائي." />
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  ...
                  onClick={() => {
                    ...
                    const schema: any = {
                      ...
                      url: promotion.slug ? `https://www.ammawag.com/promotion/${promotion.slug}` : '',
                      image: promotion.imageUrl || '',
                      price: promotion.discountValue != null ? String(promotion.discountValue) : '',
                      priceCurrency: 'SAR',
                      validFrom: promotion.startDate ? new Date(promotion.startDate).toISOString().split('T')[0] : undefined,
                      validThrough: promotion.endDate ? new Date(promotion.endDate).toISOString().split('T')[0] : undefined,
                      availability: promotion.active === false ? 'https://schema.org/OutOfStock' : 'https://schema.org/InStock',
                    };
